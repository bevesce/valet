...
def move_images(path):
	if c.extension_in(path, c.image_extensions + ['mp4']):
		photoPath = path
		cdate = datetime.utcfromtimestamp(os.path.getmtime(photoPath))
		imgDir = '/Users/bvsc/Dropbox/Images/Photos/{year:04d}/{year:04d}-{month:02d}/'.format(year=cdate.year, month=cdate.month)
		a.move(photoPath, imgDir)
		return True

valet.run('/Users/bvsc/Dropbox/Images/Photos/')